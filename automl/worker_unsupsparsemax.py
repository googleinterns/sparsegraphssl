...
import gc
import numpy as np
from automl.worker_softsparsemax import PyTorchWorker as SoftSparseMaxWorker


class PyTorchWorker(SoftSparseMaxWorker):
    ...
    def get_configspace():
        ...
        model = CSH.CategoricalHyperparameter('model', ['unsupsparsegat'])
        datasets = ['cora', 'citeseer', 'pubmed']
        ...
        labeling_rate = CSH.CategoricalHyperparameter('labeling_rate', [0.01, 0.02, 0.05, 0.10, 0.2, 1])
        patience = CSH.CategoricalHyperparameter('patience', [100])

        # alpha_sparsemax = CSH.CategoricalHyperparameter('alpha_sparsemax', list(np.arange(0, 1.1, .1)))
...
def add_sparsegat_config(cs, model):
    ...
    hyperparameters = [in_drop, attention_loss, att_last_layer]
